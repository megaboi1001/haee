...
**Deployment status:** the sandbox stack has now deployed successfully end-to-end in `434481793703` (eu-central-1). Iterations caught and fixed four template bugs: `ClientVpnEndpoint` VpcId, `Default: ""` for the S2S parameters, early-validation bucket-name collisions, and — after flow logs were dropped — `ClientVpnRouteVpc` failing with `InvalidClientVpnDuplicateRoute` because aws auto-creates a route to the VPC CIDR when a subnet is associated (`Origin: associate-target-network`; the explicit route was removed). The real Entra ID SAML metadata has been applied to the provider via `update-saml-provider`. The production stack in `440027026402` is still pending deploy — runtime concerns there (AMI availability, service quotas, IAM permissions of the deploying principal) remain unchecked until then.